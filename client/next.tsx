...
export function getPageRoutes(
  importMap: Record<
    string,
    {
      default: React.ElementType | null;
      getServerSideProps: unknown;
      path: string;
    }
  >,
  ...
  return (
    Object.keys(importMap)
      ...
      .map((path) => ({
        path: path
          // Remove /pages and .jsx or .tsx extension
          .slice(6, -4)
          // Replace [id] with :id
          .replace(/\[(\w+)\]/, (_, m: string) => `:${m}`)
          // Replace '/index' with '/'
          .replace(/\/index$/, '/'),
        // The React component (default export)
        component: importMap[path]?.default ?? null,
...
export type Context<T = unknown> = { serverSideProps?: T };
...
function fetchWithSuspense<T extends { message: string; statusCode: number }>(
  path: string,
): T | null {
  let loader: Loader<T>;
